...
class Wall extends CI_Controller {
    ...
    function index($mode='show',$section='problem',$limit=15,$page=1,$order='date_created')
    {
      $count = 0;
      if($mode == "show"){
        $posts = $this->Wall_m->getPosts($section,$limit,($page-1)*$limit,$order);
      ...
      $data = array('posts' => $posts,'mode'=>$mode, 'pages'=>(intval(($count-1)/$limit))+1, 'page'=>$page,'fb_data'=>$fb_data);
    ...
    function collapsed($post_id)
    {
      ...
      $fb_data = $this->session->userdata('fb_data');              
      $data = array('comments' => $comments, 'post_id' => $post_id,'comments_from'=>2,'fb_data'=>$fb_data);
      $this->load->view('comment',$data);
    }
            ...
            if($size<(1024*1024))
            {
              ...
              if(move_uploaded_file($tmp, $path.$actual_image_name))
              {
                ...
                if(in_array($ext, $valid_formats_img)){
                  echo "<img src='".base_url()."media/".$actual_image_name."'  class='showthumb' width='150'>";
