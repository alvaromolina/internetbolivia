...
class Wall_m extends CI_Model {
  ...
  function getPosts($section='problem',$limit,$offset,$order='date_created'){

    if($order == 'likes')
      $order_by = array($order => '-1','date_created'=>'-1'); 
    else
      $order_by = array($order => '-1'); 
    $posts = $this->mongo_db->where(array('section'=>$section))->order_by($order_by)->limit($limit)->offset($offset)->get('posts');
    return $posts;
  }
